        ...
        <div class="flex flex-1 overflow-hidden h-screen max-screen-2xl m-auto">
            <div class="p-0 lg:p-0 w-full">
                <div class="max-h-full h-full flex flex-row">
                                ...
                                <div>
                                    <button id="theme-toggle" type="button" class="text-gray-500 text-sm p-2.5">
                                        ...
                                        <svg id="theme-toggle-light-icon" class="w-5 h-5 hidden"
                                            ...
                                            stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M12 3v2m0 14v2m8.66-8.66h-2M4.34 12H2m15.66 4.34l-1.42 1.42M6.76 6.76l-1.42-1.42m12.02 12.02l-1.42-1.42M6.76 17.24l-1.42 1.42M12 8a4 4 0 100 8 4 4 0 000-8z" />
                                        </svg>
                                    </button>
                                </div>
                                <!-- switcher end -->
                                        ...
                                        <div class="cursor-pointer flex px-10"
                                            ...
                                            <div class="mr-4 relative w-12">
                                                <img class="rounded-full w-full mr-2"
                                                    ...
                                                    alt="{{ $lead->name }}">
                                                <div
                                                    class="w-3 h-3 bg-green-500 rounded-full absolute bottom-0 right-0">
                                                </div>
                                            </div>
                                            <div class="flex flex-col flex-1">
                                                <div class="flex justify-between items-center">
                                                    <div
                                                        class="text-gray-800 text-base font-semibold dark:text-gray-300">
                                                        {{ $lead->name }}</div>
                                                    <div class="text-gray-700 dark:text-gray-600 text-xs">
                                                        {{ $lead->created_at->format('H:i') }}</div>
                                                </div>
                    ...
                    <!-- right section -->
                    <section
                        class="relative max-h-full h-full bg-white rounded-lg w-full flex-col dark:bg-gray-900 lg:flex hidden">
                        <div id="allmessages"
                            class="flex-1 overflow-y-scroll p-5 scrollbar-thumb-color dark:scrollbar-thumb-color-dark scrollbar-widht space-y-5">
                            <!-- Message section -->
                            @foreach ($messages as $message)
                                <div class="flex {{ $message->is_outgoing ? 'justify-end' : 'justify-start' }}">
                                    ...
                                    @endif
                                    <div
                                        class="p-5 text-base rounded-lg inline-block max-w-xl {{ $message->is_outgoing ? 'bg-indigo-800 text-white rounded-l-lg dark:bg-indigo-900' : 'bg-gray-100 text-gray-900 rounded-r-lg dark:bg-gray-800 dark:text-white' }}">
                                        {{ $message->content }}
                                    </div>
                                </div>
                            @endforeach
